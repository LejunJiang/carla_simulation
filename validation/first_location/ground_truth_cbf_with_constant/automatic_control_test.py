...
def game_loop(args):
    ...
    npzfile = np.load("map_shifted_waypoints.npz")
    x_map = npzfile['arr_0']
    y_map = npzfile['arr_1']
    ...
    for data_number in range(0, 107):
        lane_invasion_count = 0
        collision_count = 0
        max_cte = 0.0
        sum_cte = 0.0
        ...
        try:
            ...
            perturbed_initial_position_array = deepcopy(initial_position_array)
            perturbed_initial_position_array[1] = perturbed_initial_position_array[1] + np.random.uniform(-1.0, 1.0)
            perturbed_initial_position_array[2] = perturbed_initial_position_array[2] + \
                np.random.uniform(-0.4, 0.4) / PI * 180
            world = World(client.load_world('Town06'), hud, args,
                          perturbed_initial_position_array[0], perturbed_initial_position_array[1], perturbed_initial_position_array[2])
            ...
            initial_cte = 0.0
            initial_theta_e = 0.0
            max_run_step_len = 800

            while True and run_step_len <= max_run_step_len:
                ...
                if abs(x_rear_wheel - initial_position_array[0]) < 1 and abs(y_rear_wheel - initial_position_array[1]) < 1 and run_step_len > 100:
                    break
            ...
            if run_step_len > 300:
                df = pd.read_pickle('out/results.pd')
                df_new = pd.DataFrame({'initial_x': [perturbed_initial_position_array[0]],
                                       'initial_y': [perturbed_initial_position_array[1]],
                                       'initial_theta': [perturbed_initial_position_array[2]],
                                       'initial_cte': [initial_cte],
                                       'initial_theta_e': [initial_theta_e],
                                       'lane_invasion_count': [lane_invasion_count],
                                       'collision_count': [collision_count],
                                       'complete_a_loop': [run_step_len < max_run_step_len],
                                       'max_cte': [max_cte],
                                       'mean_cte': [sum_cte / run_step_len]})
                df = df.append(df_new, ignore_index=True)
                df.to_pickle('out/results.pd')
