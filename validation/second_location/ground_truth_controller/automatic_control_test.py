...
def game_loop(args):
    ...
    for data_number in range(0, 107):
        ...
        try:
            ...
            df_1 = pd.read_pickle('out/results_1.pd')
            row_count = df_1.shape[0]
            df_row = deepcopy(df.iloc[row_count:row_count + 1, :])
            world = World(client.load_world('Town06'), hud, args,
                          df_row['initial_x'][row_count], df_row['initial_y'][row_count], df_row['initial_theta'][row_count])
            ...
            if run_step_len > 300:
                df_row['initial_cte_baseline'] = [initial_cte]
                df_row['initial_theta_e_baseline'] = [initial_theta_e]
                df_row['lane_invasion_count_baseline'] = [lane_invasion_count]
                df_row['collision_count_baseline'] = [collision_count]
                df_row['complete_a_loop_baseline'] = [run_step_len < max_run_step_len]
                df_row['max_cte_baseline'] = [max_cte]
                df_row['mean_cte_baseline'] = [sum_cte / run_step_len]
                df_1 = df_1.append(df_row, ignore_index=True)
                df_1.to_pickle('out/results_1.pd')
